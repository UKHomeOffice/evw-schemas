...
function getFieldDepartureMode(result) {
  return result.departureMode && result.departureMode.replace('Private ', '');
}
...
function getDepartureForUKDate(result) {
  ...
  if (result[departureForUKDate + 'Year'] &&
      result[departureForUKDate + 'Month'] &&
      result[departureForUKDate + 'Day'] &&
      result[departureForUKTime + 'Hour'] &&
      result[departureForUKTime + 'Minutes']) {
    date = moment.utc(
        result[departureForUKDate + 'Year'] + '-' +
        result[departureForUKDate + 'Month'] + '-' +
        result[departureForUKDate + 'Day'] + ' ' +
        formatTime(
          result[departureForUKTime + 'Hour'],
          result[departureForUKTime + 'Minutes']) + ':00', 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss');
  }
  return date === 'Invalid date' ? null : date;
}

const getPassportDetails = (result) => parsing.withoutEmpty({
...
const getJourneyDetails = (result) => parsing.withoutEmpty({
  ...
  arrivalTime: formatTime(
    result['arrivalTime' + getFieldDepartureMode(result) + 'Hour'],
    result['arrivalTime' + getFieldDepartureMode(result) + 'Minutes']
  ),
  departureForUKDate: getDepartureForUKDate(result),
  portOfArrival: result.departureMode === 'Land' ?
    result.arrivalPointLand : result['portOfArrival' + getFieldDepartureMode(result)],
...
function transformData(result) {
  return {
    ...
    passport: getPassportDetails(result),
    ...
    journey: getJourneyDetails(result),
